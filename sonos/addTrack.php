...
// takes a spotify uri for a track and adds it to the sonos queue and queue table
	...
	if ($queue->addTrack(new Spotify($_REQUEST['uri']))){
		...
		$stmt = $conn->prepare("SELECT COUNT(URI) FROM trackHistory WHERE URI = '" . $_REQUEST['uri'] . "'");
		$stmt->execute();
		...
		if ($count > 0){
			$stmt = $conn->prepare("UPDATE trackHistory SET count = count+1 WHERE URI = '" . $_REQUEST['uri'] . "'");
			$stmt->execute();
			$stmt->reset();
		}
		// if not, add to table, set count to 1, and set date
		else{
			$stmt = $conn->prepare("INSERT INTO trackHistory (URI, count, date) VALUES ('" . $_REQUEST['uri'] . "', 1, '" . date("Y-m-d") . "')");
			$stmt->execute();
			$stmt->reset();
		}
